...
class ScriptFinder(object):
    ...
    EXCLUDE_DEFAULT = ()
    INCLUDE_DEFAULT = ('*script/*.py', '*module/*.py', '*script/*.sh',
                       '*module/*.sh')
    ...
    def find(cls, where='.', exclude=None, include=None):
        ...
        fn_exclues = cls._build_neg_filter('*setup.py', '__init__.py',
                                           *exclude)
    ...
    def _find_all_script_files(cls, base_path):
        ...
        suitable = filter(
            lambda x: not os.path.isfile(
                os.path.join(os.path.dirname(x), '__init__.py')), files)
        return suitable
    ...
    def _build_neg_filter(*patterns):
        ...
        the input matches one of the patterns.
        """
        return lambda name: not any(
            fnmatchcase(name, pat=pat) for pat in patterns)
